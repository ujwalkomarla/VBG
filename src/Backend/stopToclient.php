<?php
//This REST API is used to send message to client to inform that the guide left the connection.
...
//Prepare MySQL database Query
	...
	//Execute the query onto the database
		...
		//Fetch the result from database query.
			...
			$q2 = " DELETE FROM ".$tableInDB. " WHERE guidename=?";
			...
			if($stmt1 = $conn->prepare($query1)){
				...
				if($stmt1->execute()){
					...
					if($stmt1->fetch()){//Fetch the result fo the query
						...
						$responseC["GCMserverResponse"] = array('type' => 'stopStreaming');
						$gcpm->send(null,$responseC["GCMserverResponse"]);//The Message format sent to the guide is of JSON format['type'=>'stopStreaming']
						$response["success"] = 0;
