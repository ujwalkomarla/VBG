<?php
//This REST API is used to send text command to client
...
if(empty($_POST['username']) || empty($_POST['text'])){//Verify if the REST API call is used properly
	$response["success"] = 2;
	$response["message"] = "Enter Username, msg";
	die(json_encode($response));
}
			...
			if($stmt1 = $conn->prepare($query1)){
				...
				if($stmt1->execute()){
					...
					if($stmt1->fetch()){//Fetch the result fo the query
						...
						$responseC["GCMserverResponse"] = array('type' => 'text', 'text' => $_POST['text']);
						$gcpm->send(null,$responseC["GCMserverResponse"]);//The Message format sent to the client is of JSON format['type'=>'url', 'text'=>'command']
						$response["success"] = 0;
